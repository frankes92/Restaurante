...
function comprobanteABytes($trabajo, &$err) {
    ...
    $cmd = '"' . $chrome . '" --headless=new --disable-gpu --no-sandbox --hide-scrollbars'
         ...
         . ' --virtual-time-budget=6000 --screenshot="' . $png . '" "' . $url . '" 2>&1';
    @exec($cmd, $o, $rc);

    if (!file_exists($png) || filesize($png) < 100) { $err = 'Chrome no generó la imagen del comprobante'; @unlink($png); return null; }
    ...
    foreach ($trabajos as $trabajo) {
        ...
        try {
            ...
            if (($trabajo['tipo'] ?? '') === 'comprobante') {
                // Boleta/factura/nota con diseño exacto (imagen del sistema)
                $bytes = comprobanteABytes($trabajo, $err);
            } else {
